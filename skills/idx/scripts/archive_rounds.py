#!/usr/bin/env python3
"""IDX archive_rounds — move IDX rounds older than retention window into archive/{YYYY-Q[1-4]}/.

Mirrors TCX archive script with IDX round_id format `IDX-{YYYYMMDD}-{NNN}`.

Default invocation (from project root that contains `.idx/`):

    python skills/idx/scripts/archive_rounds.py                          # 90-day retention
    python skills/idx/scripts/archive_rounds.py --retain-days 180        # custom
    python skills/idx/scripts/archive_rounds.py --dry-run                # preview only
    python skills/idx/scripts/archive_rounds.py --idx-root /path/.idx    # override path

Behavior:
- Scans `.idx/rounds/IDX-{YYYYMMDD}-{NNN}/` directories.
- Any round whose date is older than `retain_days` is moved into `.idx/archive/{YYYY-Q[1-4]}/`.
- The current latest round (per `.idx/index.yaml`) is never archived even if its date is old.
- After moving, updates `.idx/index.yaml`: removes archived rounds from the `rounds:` list and
...
ROUND_ID_RE = re.compile(r"^IDX-(\d{8})-(\d{3})$")


def parse_round_date(round_id: str):
    """Return datetime.date for an IDX-YYYYMMDD-NNN id, or None if malformed."""
...
def archive(idx_root: Path, retain_days: int, dry_run: bool, verbose: bool):
    rounds_dir = idx_root / "rounds"
    archive_dir = idx_root / "archive"
    index_path = idx_root / "index.yaml"

    if not rounds_dir.exists():
        print(f"[idx-archive] no rounds dir at {rounds_dir}; nothing to do.")
        return 0

    index = load_yaml(index_path) if index_path.exists() else {"idx_output": {"rounds": []}}
    output = index.setdefault("idx_output", {})
    ...
    if not candidates:
        print(f"[idx-archive] no rounds older than {retain_days} days (cutoff={cutoff}).")
    ...
    print(f"[idx-archive] retention={retain_days}d, cutoff={cutoff}, moving {len(candidates)} round(s).")
    moved_ids = set()
    for round_path, round_id, round_date in candidates:
        ...
        if not readme.exists():
            readme.write_text(
                "Archived IDX rounds. Do not modify in place; clone if you need to replay.\n",
                encoding="utf-8",
            )
        if target.exists():
    ...
    if not dry_run and moved_ids:
        ...
        dump_yaml(index_path, index)
        print(f"[idx-archive] index updated; archived={len(moved_ids)}, in_archive={ap['rounds_in_archive']}.")

    return len(moved_ids)


def main():
    ap = argparse.ArgumentParser(description="Archive IDX rounds older than retention window.")
    ap.add_argument("--idx-root", default=".idx", help="Path to .idx/ directory (default: ./.idx)")
    ...
    idx_root = Path(args.idx_root)
    if not idx_root.exists():
        print(f"[idx-archive] {idx_root} does not exist.", file=sys.stderr)
        return 1
    archive(idx_root, args.retain_days, args.dry_run, args.verbose)
    return 0
