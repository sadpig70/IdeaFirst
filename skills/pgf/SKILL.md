...
- **Epigenetic PPR** (v2.4) — 컨텍스트 적응 실행, extract-ppr.py 자동 통합
...
| `{SKILL_DIR}/loop/loop-reference.md` | Stop Hook loop engine — node selection, prompt composition, error recovery |
...
| `{SKILL_DIR}/reference/review-reference.md` | **v2.5** review mode — iterative analysis, prioritization, fix, re-verification |
| `{SKILL_DIR}/reference/evolve-reference.md` | **v2.5** evolve mode — self-evolution cycle, capability audit, stabilization detection |
...
| `{SKILL_DIR}/reference/delegate-reference.md` | **v2.2** DELEGATE 모드 — AI-to-AI 핸드오프, authority bounds, delegation chain |
| `{SKILL_DIR}/reference/micro-reference.md` | **v2.2** MICRO 모드 — ≤10 노드 제로 오버헤드 실행, 자동 승격 |
| `{SKILL_DIR}/reference/session-learning-reference.md` | **v2.2** Session Learning — 세션 간 학습, 패턴 누적, 전략 적응 |
...
Discovery Engine's 8 personas are independently defined as agent files in `{SKILL_DIR}/agents/`:
...
| `discover` | "발견해줘", "아이디어" | IdeaFirst 7-stage × 8 personas → idea discovery |
...
On `/PGF loop start`: (1) Verify WORKPLAN exists (2) Run `init-loop.py` (3) Determine mode (DESIGN exists → Standard / absent → Lightweight) (4) Select first node + load execution spec (5) Begin implementation. Afterwards, Stop Hook automatically injects the next node.
...
Invoke 8 `{SKILL_DIR}/agents/pgf-persona-p*.md` agents in parallel (model: sonnet). Integrate results from each stage → save to `.pgf/discovery/{step}.md`. HAO principle: do not enforce output format, preserve originals unedited.
...
- [ ] **Code Quality**: 변경된 코드를 `/simplify`로 재사용/품질/효율성 검증
